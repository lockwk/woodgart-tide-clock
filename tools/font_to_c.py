...
def render_font(ttf_path, size, wght=None, opsz=None):
    ...
    glyphs      = []
    bitmaps     = bytearray()
    raw_advance = {}   # float px — kept for accurate kern computation

    for char in CHARSET:
        adv_raw = font.getlength(char)
        raw_advance[char] = adv_raw
        # Store advance width as 26.6 fixed-point (pixels * 64) so the C
        # renderer can accumulate sub-pixel positions without rounding drift.
        adv_w_fp = round(adv_raw * 64)
    ...
    # -----------------------------------------------------------------------
    # Kern table — NxN matrix of int8 adjustments (pixels).
    # kern[i][j] = how many pixels to add to adv_w[i] when followed by char j.
    # Computed by comparing font.getlength(pair) against the sum of individual
    # advance widths.  Most entries will be 0; Inter has a few hundred non-zero
    # pairs (e.g. F-T, A-V, T-space, etc.).
    # -----------------------------------------------------------------------
    n = len(CHARSET)
    kern = [[0] * n for _ in range(n)]
    # Use raw float advances for kern computation so that we compare
    # pair_w against the true (unrounded) individual advances.
    adv_raw_list = [raw_advance[ch] for ch in CHARSET]

    for i, ci in enumerate(CHARSET):
        for j, cj in enumerate(CHARSET):
            pair_w   = font.getlength(ci + cj)   # float px
            adjustment = pair_w - adv_raw_list[i] - adv_raw_list[j]
            # Kern stored as whole pixels (int8); typical range −10 to 0.
            kern[i][j] = max(-128, min(127, round(adjustment)))

    return glyphs, bitmaps, metrics, kern
...
def emit_c_file(name, glyphs, bitmaps, metrics, kern):
    lines = []
    ...
    for g in glyphs:
        ...
        # adv_w is 26.6 fixed-point (pixels * 64); divide by 64 for display
        lines.append(
            f"    {{ .ch={cl:6s}, .width={g['width']:3d}, .height={g['height']:3d}, "
            f".ofs_x={g['ofs_x']:4d}, .ofs_y={g['ofs_y']:4d}, "
            f".adv_w={g['adv_w']:5d}, .data_offset={g['data_offset']:6d} }},"
            f"  /* {g['char']}  adv={g['adv_w']/64:.2f}px */"
        )
    lines.append("};")
    lines.append("")
    ...
    # ---- kern table — flat NxN row-major, int8 ----
    n = len(glyphs)
    lines.append(f"/* kern_table[i*{n}+j]: pixel adjustment after glyph i, before glyph j */")
    lines.append(f"static const int8_t {name}_kern[{n} * {n}] = {{")
    for i, row in enumerate(kern):
        vals = ", ".join(f"{v:4d}" for v in row)
        lines.append(f"    {vals},  /* '{CHARSET[i]}' */")
    lines.append("};")
    lines.append("")
    ...
    lines.append(f"    .kern_table  = {name}_kern,")
...
def main():
    ...
    for (name, ttf_path, size, wght, opsz) in FONTS:
        ...
        glyphs, bitmaps, metrics, kern = render_font(ttf_path, size, wght, opsz)

        c_path = os.path.join(OUT_DIR, f"{name}.c")
        h_path = os.path.join(OUT_DIR, f"{name}.h")

        with open(c_path, "w") as f:
            f.write(emit_c_file(name, glyphs, bitmaps, metrics, kern))
